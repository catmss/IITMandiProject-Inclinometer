...
uint32_t count_mpu = 0,sumCount = 0;// used to control display output rate
float deltat = 0.0f;  // used to control display output rate
...
double endmillis=0;
double power=0;
double energy= 0.0000;
double energy5= 0.0000;
//import processing.serial.*;        // will store last time LED was updated
double present_time1=0;
double pitch_Mahony=0;
double Kprev_pitch = 0;
...
void setup() {
  ...
  Serial.println("Time, Temp1, Temp2, Voltage, Current, Power, Energy");
...
//  TWBR = 12;  // 400 kbit/sec I2C speed
  ...
  {  
    ...
    Serial.println("Self Testing MPU9250...");
   ...
   // magcalMPU9250(magBias,magScale); 
    ...
    accX=(float)accelCount[0];
    accY=(float)accelCount[1];
    accZ=(float)accelCount[2];
...
void loop()
{
    ...
    gx = (float)gyroCount[0]*gRes*PI/180.0f;  // get actual gyro value, this depends on scale being set
    gy = (float)gyroCount[1]*gRes*PI/180.0f;  
    gz = (float)gyroCount[2]*gRes*PI/180.0f;   

    accX=(float)accelCount[0];
    accY=(float)accelCount[1];
    accZ=(float)accelCount[2];

    gyroX = (float)gyroCount[0];
    gyroY = (float)gyroCount[1];
    gyroZ = (float)gyroCount[2];
...
**/
  ...
  lastUpdate = Now;
    ...
    currentvalue=analogRead(currentsenspin);

    Ivalue=currentvalue*100.00*5.0/1024.0;
    Vvalue=sensorValue*5.0*100.00/1024.0;
    Vavg=Vavg+Vvalue;
    Iavg=Iavg+Ivalue;
    count++;    
...
double pitch_Kalman= -kalAngleY;   //+ZERO_ERROR;

 if (abs(present_time1-endmillis)>delay_time){
    
    double tim=(abs(present_time1-endmillis))/1000;
   
    double energy1 =  (tim*(Vavg*Iavg)/count)/3600;
    ...
    pitch_Mahony= -filter.getPitch();
    double second=present_time1/1000.0;
    Serial.print(second);
    Serial.print("\t");
    for (int i = 0; i < oneWireCount; i++) {
    ...
    if(second>120.0f){
      if(abs(Kprev_pitch - pitch_Kalman)>=2.0f ){
        final_pitch = pitch_Mahony-ZERO_ERROR;
        if(abs(pitch_Kalman-pitch_Mahony)>=4.0){
          final_pitch = (pitch_Kalman-ZERO_ERROR);
        }
      }
      else{
        final_pitch = (pitch_Kalman-ZERO_ERROR);
    ...
    endmillis= present_time1;
    

    }
        Kprev_pitch = pitch_Kalman;
...
void readBytes(uint8_t address, uint8_t subAddress, uint8_t count_mpu, uint8_t * dest)
{  
  Wire.beginTransmission(address);   // Initialize the Tx buffer
  Wire.write(subAddress);            // Put slave register address in Tx buffer
  Wire.endTransmission(false);       // Send the Tx buffer, but send a restart to keep connection alive
  uint8_t i = 0;
        Wire.requestFrom(address, count_mpu);  // Read bytes from slave register address 
